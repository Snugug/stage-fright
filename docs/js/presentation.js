...
async function receivePresentationControls(store) {
...
function addPresentationConnection(connection, store) {
...
function advancePresentation(connection, goto) {
  if (connection) {
    connection.send(JSON.stringify({
      goto
    }));
...
function buildNotesPreviewLink(index) {
...
function updateNotes(notes, index, current) {
...
function timing(parent) {
  ...
  let start = new Date(); // Update to begin

  update(); // Update every second
